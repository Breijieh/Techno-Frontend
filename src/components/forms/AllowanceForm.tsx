...
import type { MonthlyAllowance, StatusType, Employee } from '@/types';
import type { TransactionTypeResponse } from '@/lib/api/transactionTypes';

interface AllowanceFormProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (data: Partial<MonthlyAllowance>) => Promise<void>;
  initialData?: MonthlyAllowance | null;
  loading?: boolean;
...
export default function AllowanceForm({
  ...
  const [formData, setFormData] = useState<Partial<MonthlyAllowance>>({
    employeeId: 0,
    monthYear: '',
    transactionType: 0,
    transactionAmount: 0,
  ...
  const validate = (): boolean => {
    ...
    if (!formData.transactionType || formData.transactionType <= 0) {
      newErrors.transactionType = 'نوع المعاملة مطلوب';
    }
    if (!formData.transactionAmount || formData.transactionAmount <= 0) {
      newErrors.transactionAmount = 'يجب أن يكون مبلغ المعاملة أكبر من 0';
    }
  ...
  // Parse month/year from date picker
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDateFns}>
      <AnimatedDialog
        open={open}
        onClose={onClose}
        title={isEdit ? 'تعديل البدل' : 'إضافة بدل جديد'}
        maxWidth="md"
        ...
        <Box sx={{ py: 1 }}>
          ...
          <SmartRow>
            <SmartField>
              <AnimatedSelect
                label="نوع المعاملة"
                value={formData.transactionType || 0}
                onChange={(val: string | number) => setFormData({ ...formData, transactionType: val as number })}
                options={[{ value: 0, label: 'اختر النوع' }, ...transactionTypeOptions]}
            ...
            <SmartField>
              <AnimatedNumberField
                ...
                onChange={(val: number | string) => setFormData({ ...formData, transactionAmount: val === '' ? 0 : Number(val) })}
