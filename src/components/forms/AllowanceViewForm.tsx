...
import type { MonthlyAllowance, Employee } from '@/types';
import type { TransactionTypeResponse } from '@/lib/api/transactionTypes';
import { formatNumber } from '@/lib/utils/numberFormatter';

interface AllowanceViewFormProps {
  open: boolean;
  onClose: () => void;
  data: MonthlyAllowance | null;
  employees: Employee[];
  transactionTypes: TransactionTypeResponse[];
}

export default function AllowanceViewForm({
  open,
  onClose,
  data,
  employees,
  transactionTypes,
}: AllowanceViewFormProps) {
  if (!data) return null;

  const employee = employees.find((e) => e.employeeId === data.employeeId);
  const employeeName = employee?.fullName || `رقم: ${data.employeeId}`;

  const transactionType = transactionTypes.find((t) => t.typeCode === data.transactionType);
  const transactionTypeName = transactionType?.typeName || `نوع #${data.transactionType}`;
  ...
  const fields = [
    ...
    {
      label: 'مبلغ المعاملة',
      value: `ر.س ${formatNumber(data.transactionAmount ?? 0)}`,
    },
    {
      label: 'الحالة',
      value: (
