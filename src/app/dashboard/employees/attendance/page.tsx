...
export default function AttendancePage() {
  ...
  const table = useMaterialReactTable({
    ...
    renderTopToolbar: ({ table }) => {
      const handleExport = () => {
        // Export function contents are maintained in closure or moved out if needed, 
        // but here I need to redefine it or access it. 
        // Since it logic is simple, I'll inline it or keep it here.
        // Wait, I am replacing renderTopToolbarCustomActions which HAD the function.
        // I need to reconstruct the handleExport here.
        ...
        const exportData = rows.map((row) => {
          ...
          const employeeName = response?.employeeName || response?.employeeName || `موظف ${transaction.employeeId}`;

          return {
            employeeId: transaction.employeeId,
            employeeName,
            date: new Date(transaction.attendanceDate as string | Date).toLocaleDateString('en-GB'),
            entryTime: transaction.entryTime || 'غير متوفر',
            exitTime: transaction.exitTime || 'غير متوفر',
