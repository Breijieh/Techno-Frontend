...
import AllowanceRequestForm from '@/components/forms/AllowanceRequestForm';
import AllowanceRequestViewForm from '@/components/forms/AllowanceRequestViewForm';
...
export default function AllowanceRequestsPage() {
  ...
  const [allowanceResponses, setAllowanceResponses] = useState<AllowanceDetailsResponse[]>([]); // Store raw responses for employee names
  const [selectedRequest, setSelectedRequest] = useState<AllowanceRequest | null>(null);
  ...
  const getEmployeeName = useCallback((empId: number): string => {
    const response = allowanceResponses.find(r => r.employeeNo === empId);
    return response?.employeeName || `الموظف ${empId}`;
  }, [allowanceResponses]);

  // Fetch allowance requests from API
  const { execute: fetchAllowanceRequests, loading: loadingAllowances, error: allowancesError } = useApiWithToast(
    async () => {
      const params: Parameters<typeof allowancesApi.getAllAllowances>[0] = {
      ...
      const response = await allowancesApi.getAllAllowances(params);

      const mappedAllowances = mapAllowanceResponseListToAllowanceRequestList(response.content);
      setAllowanceRequests(mappedAllowances);
      setAllowanceResponses(response.content); // Store raw responses for employee names
      setTotalElements(response.totalElements);

      return response;
    },
    { silent: true, errorMessage: 'فشل تحميل طلبات البدلات' }
  );

  // Fetch allowance requests when pagination changes
  useEffect(() => {
  ...
  const handleView = (request: AllowanceRequest) => {
    setSelectedRequest(request);
    setIsViewModalOpen(true);
  };

  const handleApproveClick = (request: AllowanceRequest) => {
    setSelectedRequest(request);
    setApprovalAction('approve');
    setIsApprovalModalOpen(true);
  };

  const handleRejectClick = (request: AllowanceRequest) => {
    setSelectedRequest(request);
  ...
  const { execute: submitAllowanceRequest, loading: submittingAllowance } = useApiWithToast(
    async (data: Partial<AllowanceRequest>) => {
      if (!data.allowanceAmount || !data.allowanceType || !data.employeeId || !data.reason) {
      ...
      // Map allowanceType string to typeCode
      // For now, use a simple mapping - this should be improved to fetch from backend
      const typeCodeMap: Record<string, number> = {
        'Salary Increase': 10,
        'Special Allowance': 11,
      ...
      // Refresh the list
      await fetchAllowanceRequests();
      setIsAddModalOpen(false);
      setSelectedRequest(null);
    },
    { successMessage: 'تم إرسال طلب البدل بنجاح' }
  );

  // Approve/reject allowance request
  const { execute: handleApprovalAction, loading: processingApproval } = useApiWithToast(
    async (action: 'approve' | 'reject', notes?: string) => {
      ...
      if (action === 'approve') {
        await allowancesApi.approveAllowance(selectedRequest.requestId, {
          approverNo,
        });
      } else {
        if (!notes || notes.trim() === '') {
        ...
        await allowancesApi.rejectAllowance(selectedRequest.requestId, {
          approverNo,
    ...
    { successMessage: approvalAction === 'approve' ? 'تمت الموافقة على طلب البدل بنجاح' : 'تم رفض طلب البدل بنجاح' }
  );

  const columns = useMemo<MRT_ColumnDef<AllowanceRequest>[]>(
    () => [
      {
      ...
      },
      {
        ...
        size: 200,
        meta: {
          getFilterLabel: (row: AllowanceRequest) => {
            const employeeName = (row as AllowanceRequest & { employeeName?: string }).employeeName;
        ...
        Cell: ({ row }) => {
          ...
          const employeeName = (row.original as AllowanceRequest & { employeeName?: string }).employeeName;
      ...
      <TableToolbarWrapper table={table}>
        <Box sx={{ display: 'flex', gap: 1 }}>
          ...
          <Tooltip title="تحديث البيانات">
            <IconButton
              onClick={() => fetchAllowanceRequests()}
              sx={{
                color: '#0c2b7a',
                '&:hover': {
    ...
    >
      {loadingAllowances ? (
        ...
        <Alert severity="error" sx={{ m: 2 }}>
          {typeof allowancesError === 'string' ? allowancesError : (allowancesError as { message?: string })?.message || 'فشل تحميل طلبات البدلات'}
        </Alert>
      ) : (
  ...
  return (
    <>
        ...
        <Box sx={{ flex: 1, padding: '32px', backgroundColor: '#F8F9FC' }}>
          ...
          >
            ...
            <Typography sx={{ fontSize: '14px', color: '#6B7280' }}>
              إدارة زيادات الرواتب والبدلات الخاصة مع التحديثات التلقائية
            </Typography>
          </Box>
      ...
      <AllowanceRequestForm
        ...
        onSubmit={submitAllowanceRequest}
        loading={submittingAllowance}
      />

      <AllowanceRequestViewForm
      ...
      <ApprovalDialog
        ...
        title={selectedRequest ? `طلب البدل #${selectedRequest.requestId}` : 'طلب البدل'}
        message={
          selectedRequest
            ? `هل أنت متأكد أنك تريد ${approvalAction === 'approve' ? 'الموافقة على' : 'رفض'} طلب البدل هذا؟`
            : 'هل أنت متأكد أنك تريد تنفيذ هذا الإجراء؟'
        }
        itemName={selectedRequest ? `المبلغ: ر.س ${selectedRequest.allowanceAmount.toLocaleString('ar-SA')}` : undefined}
